	...
	while (!quit) {
		while (SDL_PollEvent(&e) != 0) {
			if (e.type == SDL_QUIT) {
				...
				// Handle text input to update num1, num2, or operation
				validInput = true; // Update based on actual input validation
			}
			else if (e.type == SDL_MOUSEBUTTONDOWN) {
				// Calculate the result if inputs are valid
				if (validInput) {
					if (operation == '+') result = std::to_string(num1 + num2);
					else if (operation == '-') result = std::to_string(num1 - num2);
